...
function module.encrypt(msg,code)
	...
	for i,v in pairs( string.split(msg,'') ) do
		...
		local id = string.byte(v)*(i)
		id *= math.sin( tonumber(string.sub(ncode,1,1) )/ncode^.5 )*(tonumber(string.sub(ncode,2,2) )/2)
		id += math.cos( tonumber(string.sub(ncode,4,4) )/ncode^.5 )
